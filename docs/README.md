...
- ✅ **小游戏「猫猫接金币」**：右键 →「小游戏…」，接金币躲炸弹，60 秒结算兑换专注币（每天上限防刷）
...
- **v4.0.1（已完成）**：互动玩法（摸头/投喂/打工时钟/每日打卡/小游戏/空间场景切换）
- **v4.0.2**：AI 对话（可选）　- **v4.0.3**：Q 版/贴纸生成（可选）　- **v4.0.4**：Live2D（远期）
- **v4.1**：生态（创意工坊/插件 SDK/MOD 制作器/i18n）
- **v4.2**：客户端交互界面大改版（学习 GitHub 优秀案例 + 渲染方案评估 + 全窗口现代风格）
